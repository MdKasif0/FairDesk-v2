...
import { ChevronLeft, ChevronRight, User, MapPin, ArrowRight, Star, CalendarX } from 'lucide-react';
...
export default function CalendarView({
  ...
  const assignmentsForCurrentMonth = useMemo(() => {
    return assignments.filter(a => isSameMonth(new Date(a.date), currentMonth));
  ...
  return (
    <Card className="shadow-lg">
      ...
      <CardContent>
        {assignmentsForCurrentMonth.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-96 text-muted-foreground space-y-4">
             ...
             <p className="text-lg">No assignments scheduled for this month.</p>
          </div>
        ) : (
          <TooltipProvider>
